...
И прокинуть порты (в другом терминале, очевидно) следующим образом:

```bash
ssh -L 32318:localhost:32318 s368090@se.ifmo.ru -p 2222
```

Теперь открываете в браузере: `http://localhost:32318/MVC-GeoValidator/` (или любой другой порт, на который вы прокинули)
